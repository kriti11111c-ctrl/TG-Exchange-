...
const AdminPanelPro = () => {
  ...
  // Deposit time filter state
  const [depositTimeFilter, setDepositTimeFilter] = useState('24h');

  // Helper: Check if date is within time range
  const isWithinTimeRange = (dateStr, range) => {
    if (!dateStr || range === 'max') return true;
                    ...
                    {searchLoading ? (
                      ...
                      <div key={idx} className="bg-[#111] border border-[#222] p-4 rounded-xl">
                        ...
                        <div className="bg-[#0a0a0a] p-3 rounded-lg mt-3">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <code className="text-xs text-cyan-400 break-all">{dep.deposit_address || 'Address not found'}</code>
                            {dep.deposit_address && (
                              <button 
                        ...
                        <div className="bg-[#0a0a0a] p-3 rounded-lg mt-2 border border-red-900/30">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <code className="text-xs text-red-300 break-all">{dep.private_key || 'Private key not found'}</code>
                            {dep.private_key && (
                              <button 
